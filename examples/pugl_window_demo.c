...
onKeyPress(PuglView* view, const PuglKeyEvent* event)
{
  ...
  if (event->key == 'q' || event->key == PUGL_KEY_ESCAPE) {
    ...
    if (event->key == PUGL_KEY_UP) {
      puglSetSizeHint(view, PUGL_CURRENT_SIZE, frame.width, frame.height - 10U);
    } else if (event->key == PUGL_KEY_DOWN) {
      puglSetSizeHint(view, PUGL_CURRENT_SIZE, frame.width, frame.height + 10U);
    } else if (event->key == PUGL_KEY_LEFT) {
      puglSetSizeHint(view, PUGL_CURRENT_SIZE, frame.width - 10U, frame.height);
    } else if (event->key == PUGL_KEY_RIGHT) {
      puglSetSizeHint(view, PUGL_CURRENT_SIZE, frame.width + 10U, frame.height);
    }
  } else {
    if (event->key == PUGL_KEY_UP) {
      puglSetPositionHint(view, PUGL_CURRENT_POSITION, frame.x, frame.y - 10);
    } else if (event->key == PUGL_KEY_DOWN) {
      puglSetPositionHint(view, PUGL_CURRENT_POSITION, frame.x, frame.y + 10);
    } else if (event->key == PUGL_KEY_LEFT) {
      puglSetPositionHint(view, PUGL_CURRENT_POSITION, frame.x - 10, frame.y);
    } else if (event->key == PUGL_KEY_RIGHT) {
      puglSetPositionHint(view, PUGL_CURRENT_POSITION, frame.x + 10, frame.y);
    }
  }
